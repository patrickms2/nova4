...
>
    ...
    {{-- Thumbnail/Icon --}}
    @if($item->isFolder())
        @svg('heroicon-o-folder', 'w-8 h-8 shrink-0 text-primary-500')
    @else
        <div class="relative h-10 w-16 shrink-0 overflow-hidden rounded bg-gray-100 dark:bg-gray-700">
            ...
            @else
                <div class="flex h-full items-center justify-center">
                    @if($item->isVideo())
                        @svg('heroicon-o-video-camera', 'w-5 h-5 text-red-500 dark:text-red-400')
                    @elseif($item->isImage())
                        @svg('heroicon-o-photo', 'w-5 h-5 text-blue-500 dark:text-blue-400')
                    @elseif($item->isDocument())
                        @svg('heroicon-o-document-text', 'w-5 h-5 text-green-600 dark:text-green-400')
                    @elseif($item->isAudio())
                        @svg('heroicon-o-musical-note', 'w-5 h-5 text-purple-500 dark:text-purple-400')
                    @else
                        @svg('heroicon-o-document', 'w-5 h-5 text-gray-500 dark:text-gray-400')
                    @endif
                </div>
            @endif
        </div>
