    ...
    <div class="flex flex-col gap-3">
        @if($item->isFolder())
            <div class="flex aspect-video items-center justify-center rounded-md bg-gray-100 dark:bg-gray-700">
                @svg('heroicon-o-folder', 'w-12 h-12 text-primary-500')
            </div>
        @else
            <div class="relative aspect-video overflow-hidden rounded-md bg-gray-100 dark:bg-gray-700">
                ...
                @else
                    <div class="flex h-full items-center justify-center">
                        @if($item->isVideo())
                            @svg('heroicon-o-video-camera', 'w-12 h-12 text-red-500 dark:text-red-400')
                        @elseif($item->isImage())
                            @svg('heroicon-o-photo', 'w-12 h-12 text-blue-500 dark:text-blue-400')
                        @elseif($item->isDocument())
                            @svg('heroicon-o-document-text', 'w-12 h-12 text-green-600 dark:text-green-400')
                        @elseif($item->isAudio())
                            @svg('heroicon-o-musical-note', 'w-12 h-12 text-purple-500 dark:text-purple-400')
                        @else
                            @svg('heroicon-o-document', 'w-12 h-12 text-gray-500 dark:text-gray-400')
                        @endif
                    </div>
                @endif
                @if($item->isVideo())
                    <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
                        @svg('heroicon-o-play', 'w-8 h-8 text-white')
                    </div>
                @endif
            </div>
        @endif
