...
<div
    ...
    @open-remesa-modal.window="$wire.openRemesaModal()"
    @open-import-modal.window="$wire.openImportModal()"
        ...
        >
            <x-ui.card variant="sectioned">
                <x-ui.card-content class="flex items-center gap-3 px-4 py-3 border rounded-xl border-neutral-100 bg-neutral-50">
                    ...
                    <div>
                        <div class="text-2xl font-semibold leading-tight tabular-nums">{{ $totalFacturas }}</div>
                        <div class="text-sm text-muted-foreground">Facturas emitidas</div>
                    </div>
                </x-ui.card-content>
            </x-ui.card>
<x-ui.card variant="sectioned">
                <x-ui.card-content class="flex items-center gap-3 px-4 py-3 border rounded-xl border-neutral-100 bg-neutral-50">
                    ...
                    <div>
                        <div class="text-2xl font-semibold leading-tight tabular-nums">{{ number_format($totalBase, 0, ',', '.') }} €</div>
...
<x-ui.card variant="sectioned">
                <x-ui.card-content class="flex items-center gap-3 px-4 py-3 border rounded-xl border-neutral-100 bg-neutral-50">
                    ...
                    <div>
                        <div class="text-2xl font-semibold leading-tight tabular-nums">{{ number_format($totalIgic, 0, ',', '.') }} €</div>
                        <div class="text-sm text-muted-foreground">IGIC total</div>
                    </div>
                </x-ui.card-content>
            </x-ui.card>
<x-ui.card variant="sectioned">
                <x-ui.card-content class="flex items-center gap-3 px-4 py-3 border rounded-xl border-neutral-100 bg-neutral-50">
                    ...
                    <div>
                        <div class="text-2xl font-semibold leading-tight tabular-nums">{{ number_format($totalImporte, 0, ',', '.') }} €</div>
                        <div class="text-sm text-muted-foreground">Importe total</div>
                    </div>
                </x-ui.card-content>
            </x-ui.card>
...
<div class="flex-1 overflow-auto bg-[#f7f7f5]">
        ...
        {{-- Tabla de facturas --}}
<x-ui.card class="overflow-hidden bg-white border shadow-sm border-neutral-200 rounded-2xl">        <x-ui.card-content class="p-0">
            <x-ui.table x-data="{
                selected: [],
                ...
                <x-ui.table-body>
                    @forelse($facturasList as $factura)
                        <x-ui.table-row x-data="blatMenu()"
                            ...
                            <x-ui.table-cell class="text-sm text-muted-foreground whitespace-nowrap">
                            ...
                            <x-ui.table-cell class="text-sm text-right tabular-nums">
                                {{ number_format($factura->baseimponible, 2, ',', '.') }} €
                            </x-ui.table-cell>
                            <x-ui.table-cell class="text-sm text-right tabular-nums text-muted-foreground">
                                {{ number_format($factura->impuesto, 2, ',', '.') }} €
                            </x-ui.table-cell>
                            <x-ui.table-cell class="text-sm font-bold text-right tabular-nums">
                                ...
                                <div wire:ignore class="flex items-center gap-2 ml-auto">
                                    ...
                                    <x-ui.dropdown-menu>
                                        ...
                                        <x-ui.dropdown-menu-content align="end" class="w-40">
                                            ...
                                            <x-ui.dropdown-menu-item>
                                                <x-lucide-file-text class="mr-2 size-4" />
                                            ...
                                            <x-ui.dropdown-menu-item
                                                ...
                                                <x-lucide-info class="mr-2 size-4" />
                                                Detalles VeriFactu
                                            </x-ui.dropdown-menu-item>
                                            <x-ui.dropdown-menu-separator />
                                            <x-ui.dropdown-menu-item class="text-destructive"
                                                ...
                                                <x-lucide-ban class="mr-2 size-4" />
                                                Cancelar
                                            </x-ui.dropdown-menu-item>
                                            <x-ui.dropdown-menu-item class="text-destructive"
                            ...
                            <x-ui.context-menu-content class="w-48">
                                ...
                                <x-ui.context-menu-item @click="$wire.duplicateFactura({{ $factura->id }})">
                                    <x-lucide-copy class="mr-2 size-4" />
                                    Duplicar
                                </x-ui.context-menu-item>
                                <x-ui.context-menu-separator />
                                <x-ui.context-menu-item
wire:click="crearPdf({{ $factura->id }})"
...
Crear Pdf
                                    ...
                                    <x-lucide-file-text class="mr-2 size-4" />
<a href="/facturas-pdf/{{  $factura->codfactura }}.pdf" target="_blank" class="w-full">Ver PDF 2</a>

                                </x-ui.context-menu-item>
                                <x-ui.context-menu-separator />
                                <x-ui.context-menu-item
                                    wire:click="enviaFactura({{ $factura->id }})"
                                    wire:confirm="¿Enviar esta factura?"
                                    wire:loading.attr="disabled"
                                    wire:target="enviaFactura"
                                >
                                    <x-lucide-send class="mr-2 size-4" />
                                    Enviar Factura
                                </x-ui.context-menu-item>                                <x-ui.context-menu-item
                        ...
                        <x-ui.table-row>
                            <x-ui.table-cell colspan="9" class="py-12 text-center">
                                <x-ui.empty class="p-0 border-0">
                                    <x-lucide-file-search class="mx-auto mb-2 size-10 opacity-30" />
                ...
                <x-ui.table-footer>
                    <x-ui.table-row class="font-semibold hover:bg-transparent">
                        ...
                        <x-ui.table-cell class="text-sm font-bold text-right tabular-nums text-primary">
    ...
    <x-ui.sheet entangle="$wire.entangle('showEditor')" x-cloak>
        <x-ui.sheet-content
            side="right"
            :show-close="false"
            class="flex flex-col w-screen max-w-5xl gap-0 p-0 overflow-hidden"
        >
 ...
 {{-- Cod. Factura --}}
                    <div class="flex flex-1 gap-4 mt-1 ml-4 items-top">
            ...
            <div class="px-4 py-2 border-b shrink-0 bg-muted/40">
            ...
            <div class="flex items-center justify-between px-4 py-1 border-b shrink-0 bg-muted">
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                @if(!$form['cliente_id'])
                    ...
                    <x-ui.empty class="h-full border-0 rounded-none text-muted-foreground">
                        <x-lucide-users class="mx-auto mb-1 size-10 opacity-30" />
                    ...
                    <div class="divide-y divide-border">
                        @forelse($form['lineas'] as $index => $linea)
                            <div class="grid items-center gap-1 px-3 py-1.5 hover:bg-accent/30 transition-colors"
                                ...
                                {{-- Importe (read-only) --}}
                                <div class="pr-1 text-sm font-semibold text-right tabular-nums text-foreground">
                                    {{ number_format($linea['importe'] ?? 0, 2, ',', '.') }} €
                                </div>

                                {{-- Eliminar --}}
                                <button type="button" wire:click="removeLinea({{ $index }})"
                                    class="flex items-center justify-center p-1 transition-colors rounded cursor-pointer text-destructive/60 hover:text-destructive hover:bg-destructive/10"
                                    title="Eliminar línea">
                                    <x-lucide-x class="size-3.5" />
                                </button>
                            </div>
                        @empty
                            <div class="px-4 py-8 text-sm text-center text-muted-foreground">
            ...
            {{-- Barra de totales --}}
            <div class="px-4 py-2 border-t-2 shrink-0 border-border bg-muted/50">
                <div class="flex flex-wrap items-center text-xs gap-x-6 gap-y-1">
    ...
    >
        <div class="w-full max-w-md p-6 transition-all transform scale-100 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                ...
                <button wire:click="$set('showClienteModal', false)" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
            ...
            <div class="space-y-3.5 text-xs">
                <div>
                    ...
                    <input type="text" wire:model="nuevoCliente.nombretotal" placeholder="Ej: Lanzaloe, S.L." class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">DNI / NIF / CIF</label>
                    <input type="text" wire:model="nuevoCliente.dni" placeholder="Ej: B12345678" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Email de contacto</label>
                    <input type="email" wire:model="nuevoCliente.email" placeholder="correo@cliente.com" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Teléfono</label>
                    <input type="text" wire:model="nuevoCliente.telefono" placeholder="+34 600 000 000" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Domicilio</label>
                        <input type="text" wire:model="nuevoCliente.domicilio" placeholder="Calle, Nº..." class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Población</label>
                        <input type="text" wire:model="nuevoCliente.poblacion" placeholder="Ciudad" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                </div>
            </div>

            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="$set('showClienteModal', false)" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cancelar
                </button>
                <button wire:click="saveNuevoCliente" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all bg-indigo-600 shadow-sm cursor-pointer rounded-xl hover:bg-indigo-700 shadow-indigo-600/10 active:scale-95">
                    Guardar cliente
                </button>
            </div>
        </div>
    ...
    >
        <div class="w-full max-w-md p-6 transition-all transform scale-100 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                ...
                <button wire:click="$set('showConceptoModal', false)" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <div class="space-y-3.5 text-xs">
                <div>
                    ...
                    <input type="text" wire:model="nuevoConcepto.concepto" placeholder="Ej: Servicios de desarrollo web" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Grupo / Categoría</label>
                        <input type="text" wire:model="nuevoConcepto.grupo" placeholder="Ej: Ingeniería" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Unidad de medida</label>
                        <input type="text" wire:model="nuevoConcepto.unidad" placeholder="Ej: UNID, HORA..." class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Precio (€)</label>
                        <input type="number" step="0.01" wire:model="nuevoConcepto.precio" placeholder="0.00" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Descuento %</label>
                        <input type="number" step="0.01" wire:model="nuevoConcepto.descuento" placeholder="0" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">IGIC %</label>
                        <input type="number" step="0.01" wire:model="nuevoConcepto.impuesto" placeholder="7" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Retención %</label>
                    <input type="number" step="0.01" wire:model="nuevoConcepto.retenciones" placeholder="15" class="w-full px-3 py-2 text-xs transition-all border rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
                </div>
            </div>

            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="$set('showConceptoModal', false)" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cancelar
                </button>
                <button wire:click="saveNuevoConcepto" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all bg-orange-600 shadow-sm cursor-pointer rounded-xl hover:bg-orange-700 shadow-orange-600/10 active:scale-95">
                    Guardar concepto
                </button>
            </div>
        </div>
    ...
    >
        <div class="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white border border-slate-100 p-6 shadow-2xl transition-all transform scale-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                <div class="flex items-center gap-2">
                    <span class="p-1.5 bg-indigo-50 text-orange-600 rounded-lg">
                        <x-lucide-info class="size-4" />
                    </span>
                    <h2 class="text-sm font-bold text-slate-800">Detalles VeriFactu — {{ $veriFactuDetail['codfactura'] ?? '' }}</h2>
                </div>
                <button wire:click="$set('showVeriFactuModal', false)" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
                    <x-lucide-x class="size-4" />
                </button>
            </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @if(! empty($veriFactuDetail['qr_url']))
                            <div class="col-span-2">
                                ...
                                <a href="{{ $veriFactuDetail['qr_url'] }}" target="_blank" class="text-orange-600 break-all hover:underline">{{ $veriFactuDetail['qr_url'] }}</a>
                            </div>
                        @endif
                    </div>
            ...
            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="$set('showVeriFactuModal', false)" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cerrar
                </button>
            </div>
        </div>
    ...
    >
        <div class="w-full max-w-md p-6 transition-all transform scale-100 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                ...
                <button wire:click="closeRemesaModal" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
                    <x-lucide-x class="size-4" />
                </button>
            </div>
            ...
            <div class="space-y-4 text-xs">
                @if($remesaConfirmarRegenerar)
                    <div class="p-3 border rounded-lg bg-amber-50 border-amber-200 text-amber-800">
                        <p class="mb-1 font-semibold">Esta remesa ya tiene facturas generadas</p>
                ...
                @else
                    ...
                    @if($remesaSeleccionada)
                        ...
                        @endphp
                        @if($remesaSel)
                            <div class="p-3 border rounded-lg bg-slate-50 border-slate-100">
            ...
            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="closeRemesaModal" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cancelar
                </button>
                @if($remesaConfirmarRegenerar)
                    <button wire:click="confirmarRegenerarRemesa" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all shadow-sm cursor-pointer rounded-xl bg-destructive hover:bg-destructive/90 shadow-destructive/10 active:scale-95">
                        Eliminar y regenerar
                    </button>
                @else
                    <button wire:click="iniciarGenerarRemesa" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all bg-orange-600 shadow-sm cursor-pointer rounded-xl hover:bg-orange-700 shadow-orange-600/10 active:scale-95">
                        Generar
                    </button>
                @endif
            </div>
    ...
    >
        <div class="w-full max-w-md p-6 transition-all transform scale-100 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                ...
                <button wire:click="closeDeleteModal" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
                    <x-lucide-x class="size-4" />
                </button>
            </div>
            ...
            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="closeDeleteModal" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cancelar
                </button>
                <button wire:click="ejecutarDeleteFactura" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all shadow-sm cursor-pointer rounded-xl bg-destructive hover:bg-destructive/90 shadow-destructive/10 active:scale-95">
                    Eliminar
                </button>
            </div>
        </div>
    ...
    >
        <div class="w-full max-w-md p-6 transition-all transform scale-100 bg-white border shadow-2xl rounded-2xl border-slate-100">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                ...
                <button wire:click="$set('showImportModal', false)" type="button" class="p-1 rounded-lg cursor-pointer text-slate-400 hover:text-slate-600 hover:bg-slate-50">
                    <x-lucide-x class="size-4" />
                </button>
            </div>
            ...
            <div class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <input type="file" accept="application/pdf" wire:model="pdfFile" class="w-full text-xs text-orange-700 file:mr-3 file:rounded-xl file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-orange-600 file:text-xs file:font-semibold">
                    @error('pdfFile') <p class="mt-1 text-xs text-destructive">{{ $message }}</p> @enderror
                    @if($importMessage)
                        <p class="text-xs mt-1 {{ str_starts_with($importMessage, 'Error') ? 'text-destructive' : 'text-green-600' }}">{{ $importMessage }}</p>
                    @endif
                    <div wire:loading wire:target="pdfFile" class="mt-1 text-xs text-muted-foreground">Cargando…</div>
                </div>
            </div>

            <div class="mt-5 pt-3 border-t border-slate-100 flex justify-end gap-2.5">
                <button wire:click="$set('showImportModal', false)" type="button" class="px-4 py-2 text-xs font-semibold transition-all border cursor-pointer rounded-xl border-slate-200 hover:bg-slate-50 text-slate-500 active:scale-95">
                    Cancelar
                </button>
                <button wire:click="importPdf" type="button" class="px-4 py-2 text-xs font-bold text-white transition-all bg-orange-600 shadow-sm cursor-pointer rounded-xl hover:bg-orange-700 shadow-orange-600/10 active:scale-95">
                    Importar factura
                </button>
            </div>
        </div>
