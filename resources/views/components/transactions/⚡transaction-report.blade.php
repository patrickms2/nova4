...
<div class="p-6 space-y-6">
    ...
    <div class="grid gap-6 lg:grid-cols-2">
        ...
        {{-- Donut Chart --}}
        <div class="relative bg-zinc-900 border border-zinc-800/80 rounded-2xl p-5 overflow-hidden animate-slide-up" style="animation-delay: 0.16s">
            <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-zinc-600/40 to-transparent"></div>
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h3 class="text-sm font-semibold text-white">Expense Distribution</h3>
                    <p class="text-[11px] text-zinc-500 mt-0.5">Top categories breakdown</p>
                </div>
            </div>
        ...
        <div class="relative bg-zinc-900 border border-zinc-800/80 rounded-2xl p-5 overflow-hidden animate-slide-up" style="animation-delay: 0.2s">
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    ...
                    <p class="text-[11px] text-zinc-500 mt-0.5">Income vs expense — {{ $selectedYear }}</p>
                </div>
            </div>
            <div
    ...
    <div class="relative bg-zinc-900 border border-zinc-800/80 rounded-2xl overflow-hidden animate-slide-up" style="animation-delay: 0.24s">
        <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-zinc-600/40 to-transparent"></div>
        <div class="px-5 pt-5 pb-4">
            <h3 class="text-sm font-semibold text-white">Expense by Category</h3>
            <p class="text-[11px] text-zinc-500 mt-0.5">Ranked by total spending for selected period</p>
        </div>

        <flux:table>
            <flux:table.columns>
                <flux:table.column class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400">#</flux:table.column>
                <flux:table.column class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400">Category</flux:table.column>
                <flux:table.column align="end" class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400">Total (RM)</flux:table.column>
                <flux:table.column align="end" class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400">Share</flux:table.column>
            </flux:table.columns>
            ...
            <flux:table.rows>
                @forelse($this->categoryBreakdown as $row)
                    <flux:table.row :key="$row->category_name" class="hover:bg-zinc-800/30 transition-colors">
                        <flux:table.cell>
                            @if($row->rank === 1)
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold bg-amber-500/15 text-amber-400 border border-amber-500/30">1</span>
                            @elseif($row->rank === 2)
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold bg-zinc-700/50 text-zinc-300 border border-zinc-600/50">2</span>
                            @elseif($row->rank === 3)
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold bg-orange-500/10 text-orange-400 border border-orange-500/20">3</span>
                            @else
                                <span class="text-zinc-500 text-sm pl-1">{{ $row->rank }}</span>
                            @endif
                        </flux:table.cell>
                        <flux:table.cell>
                        ...
                        <flux:table.cell align="end" class="text-rose-400">
                            {{ number_format($row->total, 2) }}
                        </flux:table.cell>
                        <flux:table.cell align="end">
                            <div class="flex items-center justify-end gap-2">
                                <div class="w-16 bg-zinc-800 rounded-full h-1.5">
                                    <div class="bg-gradient-to-r from-rose-500 to-pink-500 h-1.5 rounded-full" style="width: {{ $row->percentage }}%"></div>
                                </div>
                                <span class="text-sm text-zinc-500 w-10 text-right">{{ $row->percentage }}%</span>
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @empty
