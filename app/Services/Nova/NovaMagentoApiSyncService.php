...
final class NovaMagentoApiSyncService
{
    ...
    {
        $token = $this->tokenFor($setting);
        $baseUrl = $setting->api_url ?: $setting->base_url;
    ...
    private function tokenFor(NovaIntegrationSetting $setting): string
    {
        $credentials = $setting->credentials ?? [];
        $envGroup = (string) data_get($setting->settings, 'env_group', '');
        $credentialToken = data_get($credentials, 'access_token')
            ?: data_get($credentials, 'token');
        $token = $this->envValue($credentialToken)
            ?: $credentialToken
            ?: $this->envValue($envGroup === '' ? null : $envGroup.'_TOKEN')
            ?: $this->envValue($envGroup === '' ? null : $envGroup.'_ACCESS_TOKEN');

        return blank($token) ? '' : (string) $token;
    }
