...
class CreateInvoiceTool extends Tool
{
    ...
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            ...
            'cliente_numero' => 'nullable|integer|min:1',
    ...
    {
        return [
            'cliente_id' => $schema->integer()->description('ID del cliente al que se emite la factura. Alternativa a "cliente" y "cliente_numero".'),
            'cliente_numero' => $schema->integer()->description('Número del cliente según la lista numerada (ListNumberedClientsTool). Alternativa a "cliente_id" y "cliente".'),
            'cliente' => $schema->string()->description('Nombre (o parte del nombre) del cliente. Alternativa a "cliente_id" y "cliente_numero".'),
    ...
    /**
     * Resolve the client from its ID, number, or a name search.
     *
    ...
    private function resolveCliente(array $validated): Cliente|Response
    {
        ...
        if ($validated['cliente_numero'] ?? null) {
            $numero = $validated['cliente_numero'];
            ...
            if ($clientes->count() < $numero) {
                return Response::error("El número {$numero} no corresponde a ningún cliente. Usa ListNumberedClientsTool para ver la lista.");
            }
        ...
        if ($nombre === '') {
            return Response::error('Debes indicar cliente_id, cliente_numero o cliente (nombre).');
        }
