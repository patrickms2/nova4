...
class IncidentsRelationManager extends RelationManager
{
    ...
    {
        return $table->columns([TextColumn::make('title')->label('Incidencia'), TextColumn::make('priority')->label('Prioridad')->badge(), TextColumn::make('status')->label('Estado')->badge(), TextColumn::make('created_at')->label('Fecha')->dateTime()])->headerActions([
                            AttachAction::make(),

        CreateAction::make()->mutateDataUsing(fn (array $data): array => [...$data, 'community_id' => $this->getOwnerRecord()->community_id, 'created_by' => auth()->id(), 'updated_by' => auth()->id()])]);
    }
}
