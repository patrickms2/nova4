...
use App\Filament\App\Facturacion\Resources\RentalResource2\Pages;
use App\Filament\App\Facturacion\Resources\RentalResource2\Pages\CalendarRentals;
 ...
 * @see KML-0051 (D1)
 */
class RentalResource2 extends Resource
{
                ...
                ->schema([
                    TextEntry::make('settlement_paid')
                        ...
                        ->state(function (?Rental $record): string {
                            ...
                            return number_format($paid / 100, 2, ',', ' ') . ' RentalResource.php' .($record?->currency ?? config('rental.currency', 'EUR'));
                        }),
                    TextEntry::make('settlement_total')
                        ...
                        ->state(function (Get $get, ?Rental $record): string {
                            ...
                            return number_format($total / 100, 2, ',', ' ') . ' RentalResource.php' .($record?->currency ?? config('rental.currency', 'PLN'));
                        }),
    ...
    public static function resourceSection(): array
    {
        ...
        return [
            Section::make('Villa')
                ->schema([
                    TextEntry::make('current_rentable')
                        ->label('Villa')
                        ->state(function ($record): string {
                            if (! $record || ! $record->rentable) {
                                return '— (brak)';
                            }
                            $r = $record->rentable;
                            return $r->name ?? $r->title ?? class_basename($record->rentable_type).' #'.$r->getKey();
                        }),
    ...
    public static function formatSettlementBalance(?string $status, int $total, int $paid, string $currency): string
    {
        ...
        $balance = $total - $paid;
        $formatted = number_format(abs($balance) / 100, 2, ',', ' ') . ' RentalResource.php' .$currency;
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('total_amount')
                    ->label('Totas')
                    ->formatStateUsing(fn (int $state, Rental $r) => number_format($state / 100, 2, ',', ' ') . ' RentalResource.php' .$r->currency)
                    ->sortable(),
    ...
    public static function getPages(): array
    {
        return [
            ...
            'calendar' => CalendarRentals::route('/calendar'),
        ];
    }
