...
class Map2 extends Page implements HasForms, HasTable
{
    ...
    {
        return Hotel::query()->latest()->limit(25);
    }
...
//        $config['zoom'] = 5;
    ...
    {
        $locations = Hotel::query()->limit(5)->get()
            ->select(["id", "nombre", "lat", "lng", "location"])
            ->whereNotNull("lat")
            ->toBase()
            ->toArray();
        ...
        foreach ($locations as $location) {
            $data[] = [
                'location' => [
                    'lat' => $location['lat'] ? round(floatval($location['lat']), 6) : 0,
                    'lng' => $location['lng'] ? round(floatval($location['lng']), 6) : 0,
                ],
    ...
    {
        $results = [];
        //$results = json_decode(json_encode(Hotel::get()->toBase()->collapse()->select(["id", "nombre", "lat", "lng", "location"])->whereNotNull("lat")), true);

        $results = json_encode(
            Hotel::query()->limit(5)->get()
                ->select(["id", "nombre", "lat", "lng", "location"])
                ->whereNotNull("lat")
                ->toBase()
                ->toArray(),
        ...
        $results = str_replace("'", "", trim($results));

        $array = json_decode($results, true); //? $array
    ...
    private function getUserAttendance(): array
    {
    ...
    public static function table(Table $table): Table
    {
        return
            $table
                ->query(Hotel::query()->with('usuario'))
                ->columns([
                    TextColumn::make('id')
                    ...
                    TextColumn::make('usuario.nombre')
                        ...
                        ->searchable()
                        ->label('Nombre'),
                    BadgeColumn::make('usuario.municipio.nombre')
                        ->formatStateUsing(fn(string $state): string => strtoupper($state))
                        ->disableClick()
                        ->extraAttributes(['style' => 'font-weight: bold'])
                        ->colors([
                            'danger' => fn($record) => $record->municipio_id === 2,
                            'warning' => fn($record) => $record->municipio_id === 1,
                            'success' => fn($record) => $record->municipio_id === 3,
                            'info' => fn($record) => $record->municipio_id === 5,
                            'gray' => fn($record) => $record->municipio_id === 4,
                        ]),
                ...
                ->filters([
                        ...
                        Tables\Filters\SelectFilter::make('usuario.municipio_id')
                            ->label('Municipio')
                            ->options(function () {
                                // Exclude the current category if editing
                                $query = Municipio::query()->where('estado', '=', 1);
                ...
                ->recordActions([
                    ...
                    Action::make('Ir')
                        ->button()
                        ->alpineClickHandler(function (Model $record) {
                            $latLngFields = $record::getLatLngAttributes();
                            return new HtmlString(
                                sprintf("setMapCenter(%f,%f,'%s'); return false;",
                                    round(floatval($record->lat), 8),
