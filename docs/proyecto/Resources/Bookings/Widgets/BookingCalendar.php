...
class BookingCalendar extends FullCalendarWidget implements HasCalendar
{
    ...
    public function getEvents(FetchInfo $info): Collection|array|Builder
    {
        ...
        $selectedDepartmentId = $filters['booking_department_id'] ?? null;

        $serviceUserId = null;
        $departmentCalendarIds = collect();
        $departmentServiceUserIds = collect();

        if ($selectedDepartmentId) {
            $departmentCalendars = \App\Models\BookingCalendar::query()
                ->forDepartment((int) $selectedDepartmentId)
                ->get(['id', 'owner_id']);

            $departmentCalendarIds = $departmentCalendars
                ->pluck('id')
                ->filter()
                ->values();

            $departmentServiceUserIds = $departmentCalendars
                ->pluck('owner_id')
                ->filter()
                ->unique()
                ->values();
        ...
        $blockingPeriods = BookingServicePeriod::query()
            ...
            ->when(
                ! $serviceUserId && $selectedDepartmentId && $departmentServiceUserIds->isNotEmpty(),
                fn ($query) => $query->whereIn('service_user_id', $departmentServiceUserIds->all())
            )
            ->when(
                ! $serviceUserId && $selectedDepartmentId && $departmentServiceUserIds->isEmpty(),
        ...
        $bookings = Booking::query()
            ...
            ->when($serviceUserId, fn ($query) => $query->where('service_user_id', $serviceUserId))
            ->when(
                ! $selectedCalendarId && $selectedDepartmentId && $departmentCalendarIds->isNotEmpty(),
                fn ($query) => $query->whereIn('booking_calendar_id', $departmentCalendarIds->all())
            )
            ->when(
                ! $selectedCalendarId && $selectedDepartmentId && $departmentCalendarIds->isEmpty(),
                fn ($query) => $query->whereRaw('1 = 0')
            )
    ...
    protected function getSelectedDepartmentId(): ?int
    {
        $filters = $this->pageFilters ?? [];

        $departmentId = $filters['booking_department_id'] ?? null;
