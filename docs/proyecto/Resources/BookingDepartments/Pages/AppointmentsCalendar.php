...
use App\Filament\Portal\Pages\AppointmentsKanban;
...
class AppointmentsCalendar extends Page
{
    ...
    public function getCalendarPayload(): array
    {
        ...
        $meetings = TaxistaAppointment::query()
            ->with(['tipo', 'department'])
            ->where('created_by_user_id', $portalUserId ?: 0)
            ->whereBetween('scheduled_start_at', [$gridStart, $gridEnd])
            ->orderBy('scheduled_start_at')
            ->get()
            ->groupBy(fn(TaxistaAppointment $meeting): string => $meeting->scheduled_start_at?->format('Y-m-d') ?? '');
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('table')
                ...
                ->color('gray')
                ->outlined()
                ->url(ManageDepartmentCitas::getUrl()),
            Action::make('kanban')
                ->label('Kanban')
